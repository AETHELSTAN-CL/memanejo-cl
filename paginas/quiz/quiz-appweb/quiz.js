document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== Nombre real usuario (etapa pruebas) =====
  function obtenerIdentidadReal() {
    const guardada = localStorage.getItem("memanejo_identidad");
    if (guardada) return JSON.parse(guardada);

    let nombreReal = prompt("Ingresa tu nombre completo para identificar tu examen:");
    if (!nombreReal || !nombreReal.trim()) {
      nombreReal = "Sin nombre";
    }

    const identidad = { nombre: nombreReal.trim() };
